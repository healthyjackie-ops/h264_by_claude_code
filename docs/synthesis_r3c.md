...
背靠背块发射（mc_fetch S_OUT 拍接受下一 start；发射几何 mlk=mk_q+1
与写回几何 wck=mk_q 分离——首发混用导致写错块，全 MC 区域错位）+
chroma 2x2 块 3x3 窗口（c2x2 模式：5 行取数 → 3 行）。64x64 P 帧
10083 → 9481 拍；parse∥recon 流水下 MC 不总是关键路径，收益部分
被解析掩盖。回归全绿。

## Wave 15 — 双熵核（W15-d 终态）

CABAC 路径（cabac_core 算术引擎 + cabac_mb 全语法解析器）并入后：

| 指标 | P-R3e（CAVLC I/P） | W15-d（双熵 I/P） |
|---|---|---|
| 逻辑面积 | 29,746 µm² | **30,878 µm²（CABAC 仅 +1,132，+3.8%）** |
| mem 实例 | 61 | 80（+cabac nrow 92b 行缓冲、ctx 状态阵列） |
| 时序 | 300 MHz | **300 MHz 收敛**（ABC delay 3330.1ps < 3333 目标） |

整个 CABAC 解码器（引擎 + 436 上下文 + 全部 ROM 表 + MB 层 FSM）
约一千平方微米——对比 mc_core 全相位插值器的 ~11k。逐 bin 串行
天然小面积；吞吐换面积的取舍在解码器侧合理（bin 率 ≈ 时钟率）。

## 余项（按需）

recon/deblock 内部并行化、MC 与残差重建重叠（更高吞吐时再做）。
