// W15-b/c differential test: mb_top (bitreader + mb_dec + cavlc_block) vs
...
int main(int argc, char **argv) {
    ...
    while (nal_next(stream.data(), stream.size(), &pos, &n) == 0) {
        ...
        if (n.type == 7) {
            ...
            if (!pps.entropy_coding_mode || sh.is_p || sh.is_b ||
                pps.transform_8x8) {
                printf("[SKIP] not a CABAC I slice\n");
                return 1;
            }
            ...
            sd_byte = bs.bit ? bs.byte + 1 : bs.byte;   // cabac aligns
            sd_bit = 0;
            found = true;
            nal_free(&n);
            break;
    ...
    if (top.err) { printf("[FAIL] cabac_mb err state at MB %zu\n", gi); return 1; }
    if (gi != nrec) {
    ...
    if (fails == 0) {
        printf("[PASS] cabac_mb: %zu MBs bit-exact vs H264_RTL_DUMP\n", nrec);
        return 0;
    }
    return 1;
}
