#!/usr/bin/env python3
"""
Phase 19 vectors — temporal direct B frames.

Same motion synthesis with --direct temporal: colocated motion scaled by
POC distance (DSF), reference mapping through ref2poc, with and without
B-pyramid (scaled motion from reference-B colocated frames).
"""
...
OUT = ROOT / "verification" / "vectors" / "phase19"
...
def main() -> int:
    ...
    for w, h, nf, kind, qp, nbf, cavlc, nodb in CASES:
        ...
        name = (f"p19_{kind}_{w}x{h}_n{nf}_q{qp}_b{nbf}"
                f"{'_cavlc' if cavlc else ''}{'_nodb' if nodb else ''}")
        b264 = OUT / (name + ".264")
        cmd = ["x264", "--qp", str(qp),
               "--frames", str(nf), "--ref", "2", "--slices", "1",
               "--bframes", str(nbf), "--weightp", "0",
               "--direct", "temporal",
    ...
    print(f"generated {n} Phase 19 vectors (+golden yuv) under {OUT}")
    return 0
