...
| Profile | Baseline / Constrained Baseline (CAVLC) |
| 宏块 | I_4x4（9 预测模式）· I_16x16（4 模式 × CBP）· I_PCM |
...
**验证：46/46 向量对 ffmpeg (libavcodec 8.1) 逐字节 bit-exact**
（22 个 no-deblock + 18 个 deblock-ON 含 α/β offset 扫描 + 6 个 expect-fail
错误用例），外加真实照片帧抽查。`-Wall -Wextra -Wpedantic -Wshadow
-Wconversion -Werror` 零告警。
...
## 下一步（docs/roadmap.md Wave 2+）

CABAC I 帧 → High profile（Intra_8x8 + scaling list）→ 多 slice → P 帧 → RTL。
I_PCM 已实现但 x264 不产 PCM 流，待 JM 编码器补向量。
