...
| 维度 | 支持 |
|---|---|
| 码流 | Annex-B 序列（IDR + **P 帧**，多 slice），EPB 去转义 |
| Profile | Baseline / Main / **High** I 帧（CAVLC + CABAC，Intra_8x8 + 8x8 变换） |
| 宏块 | I_4x4 · I_8x8 · I_16x16 · I_PCM · **P_L0 全分区 + P_Skip（qpel MC）** |
...
**验证：126/126 向量对 ffmpeg (libavcodec 8.1) 逐字节 bit-exact**
（CAVLC/CABAC × baseline/main/high × 单/多 slice × I/P 帧序列共 120 个
...
## 下一步（docs/roadmap.md Wave 6+）

P 帧 CABAC → 多参考/B 帧 → ASO → RTL。
